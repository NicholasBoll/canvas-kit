...
> **Note:** There is a problem with `mergeProps` when it comes to React `ref` objects. A React `ref` is a special prop, similar to `key` where it is passed to a component or element as a prop, but is not accessible inside that component as a prop. All Canvas Kit compound components should forward the `ref` to the underlying DOM element. A behavior hook may or may not need access to a ref. `createHook` was created to normalize `ref` handling and remove the need to deal with `elemProps` directly.
